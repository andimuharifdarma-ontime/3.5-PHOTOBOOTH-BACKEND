...
const filterImageCache = new Map<string, string>();
const filterInFlight = new Map<string, Promise<string>>();

function filterCacheKey(imageUrl: string, filterType: string): string {
    return `${imageUrl}::${filterType}`;
}
...
export const applyFilterToImage = async (imageUrl: string, filterType: string): Promise<string> => {
    ...
    const cacheKey = filterCacheKey(imageUrl, filterType);
    const cached = filterImageCache.get(cacheKey);
    ...
    const task = new Promise<string>((resolve) => {
        ...
        img.onload = () => {
            ...
            filterImageCache.set(cacheKey, dataUrl);
            resolve(dataUrl);
        };
    ...
    filterInFlight.set(cacheKey, task);
    return task;
};

/** Prefetch all filter variants for a photo so picker thumbnails load instantly. */
export async function prefetchFiltersForImage(
    imageUrl: string,
    filterIds: string[],
): Promise<void> {
    if (!imageUrl) return;
    await Promise.all(
        filterIds.map((filterId) => applyFilterToImage(imageUrl, filterId)),
    );
}

export const FILTER_NAMES: Record<string, string> = Object.fromEntries(
    ALL_ARTISTIC_FILTERS.map((f) => [f.id, f.name]),
);

export const ALL_FILTER_IDS = ALL_ARTISTIC_FILTERS.map((f) => f.id);

/** CSS fallback for live-video preview only (static/print uses pixel-accurate filters). */
export const CSS_FILTER_FALLBACK: Record<string, string> = {
    original: "none",
    bw: "grayscale(100%)",
    sepia: "sepia(100%)",
    cool: "hue-rotate(200deg) saturate(120%)",
    warm: "hue-rotate(15deg) sepia(10%)",
    vivid: "saturate(180%) contrast(110%)",
    "vintage-warm": "sepia(40%) contrast(90%) brightness(105%)",
    cinematic: "contrast(125%) saturate(85%) brightness(95%)",
    pastel: "brightness(110%) saturate(80%) contrast(90%)",
    dramatic: "contrast(150%) brightness(90%)",
    "noir-contrast": "grayscale(100%) contrast(160%) brightness(90%)",
    "golden-hour": "sepia(30%) saturate(150%) hue-rotate(-10deg)",
    matte: "contrast(85%) brightness(105%) saturate(90%)",
    "clean-pop": "saturate(140%) brightness(105%) contrast(100%)",
    emerald: "hue-rotate(100deg) saturate(80%)",
};

export function getCssFilterFallback(filterId: string): string {
    return CSS_FILTER_FALLBACK[filterId] ?? "none";
}

export function normalizeEnabledFilters(raw: unknown): string[] {
    ...
    return list.filter(
        (id): id is string =>
            typeof id === "string" && ALL_FILTER_IDS.includes(id),
    );
}

export function drawImageWithArtisticFilter(
    ...
    applyFilterToImageData(imageData.data, filterId, w, h);
    tctx.putImageData(imageData, 0, 0);
    ctx.drawImage(temp, dx, dy, dw, dh);
}

export async function prefetchFilteredImage(
    ...
    const dataUrl = await applyFilterToImage(image.src, filterId);
    return new Promise((resolve) => {
        const filtered = new Image();
        filtered.crossOrigin = "anonymous";
        filtered.onload = () => resolve(filtered);
        filtered.onerror = () => resolve(image);
        filtered.src = dataUrl;
    });
}
