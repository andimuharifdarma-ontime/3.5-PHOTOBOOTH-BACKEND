...
function resolveSafeKioskBaseUrl(request: Request): string {
    const fallback = process.env.NEXT_PUBLIC_BASE_URL?.trim() || 'http://localhost:3000';
    const allowed = new Set(buildAllowedOrigins());
    const requestOrigin = request.headers.get('origin') || request.headers.get('referer');
    if (!requestOrigin) return fallback;

    try {
        const parsed = new URL(requestOrigin);
        return allowed.has(parsed.origin) ? parsed.origin : fallback;
    } catch {
        return fallback;
    }
}
...
export async function POST(request: Request) {
    ...
    try {
        ...
        // 2. Prepare DOKU Request with trusted callback origin only.
        const kioskBaseUrl = resolveSafeKioskBaseUrl(request);
