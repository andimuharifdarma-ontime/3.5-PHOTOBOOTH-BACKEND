...
export async function POST(request: Request) {
    try {
        ...
        const updatedSetting = await (prisma.systemSetting as any).upsert({
            where: { adminUserId: settingsAdminId },
            update: {
                // Partial update: only persist fields explicitly sent by the client
                ...(isPaymentEnabled !== undefined && {
                    isPaymentEnabled: userRole === 'ADMIN'
                        ? isPaymentEnabled
                        : (currentSetting?.isPaymentEnabled ?? true),
                }),
